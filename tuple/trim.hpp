...
namespace mplex {
    ...
    struct trim_left <std::tuple <T, List...>, Predicate, false> {
        using type = if_t <Predicate::template apply <T>::value,
    ...
    struct trim_right {
        // Abort == false
        using type = if_t <Predicate::template apply <typename std::tuple_element <std::tuple_size <Tuple>::value - 1, Tuple>::type>::value,
